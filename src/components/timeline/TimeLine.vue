...
export default {
  ...
  data() {
    return {
      ...
      description: null,
      showaddform: false,
      Timelines: this.$root.useObservable(
        this.$root.liveQuery(async () => await this.$root.db.Timeline.toArray())
      ),
    };
  },
  methods: {
    ...
    chooseItem(item) {
      this.$root.session.timeline.selected = this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Timeline.get(item.uuid)
        )
      );
    },
    addItem() {
