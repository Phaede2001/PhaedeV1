...
export default {
  ...
  data() {
    return {
      ...
      description: null,
      showaddform: false,
      Snowflakes: this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Snowflake.toArray()
        )
      ),
    };
  },
  methods: {
    ...
    chooseItem(item) {
      this.$root.session.snowflake.selected = this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Snowflake.get(item.uuid)
        )
      );
    },
    addItem() {
