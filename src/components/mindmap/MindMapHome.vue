...
export default {
  ...
  data() {
    return {
      ...
      description: null,
      showaddform: false,
      MindMaps: this.$root.useObservable(
        this.$root.liveQuery(async () => await this.$root.db.Mindmap.toArray())
      ),
    };
  },
  methods: {
    ...
    chooseItem(item) {
      this.$root.session.mindmap.selected = this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Mindmap.get(item.uuid)
        )
      );
    },
    addItem() {
