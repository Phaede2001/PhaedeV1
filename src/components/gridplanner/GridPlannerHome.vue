...
export default {
  ...
  data() {
    return {
      ...
      description: null,
      showaddform: false,
      Gridplanners: this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Gridplanner.toArray()
        )
      ),
    };
  },
  methods: {
    ...
    chooseItem(item) {
      this.$root.session.gridplanner.selected = this.$root.useObservable(
        this.$root.liveQuery(
          async () => await this.$root.db.Gridplanner.get(item.uuid)
        )
      );
    },
    addItem() {
