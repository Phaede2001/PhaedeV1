<template>
        ...
        <tbody>
          <tr v-for="(item, index) in toollist" :key="index">
            <td>
              ...
              <i>
                <br />
                {{ $root.niceDate(item.lastupdated) }}
              </i>
            </td>
            <td style="width: 50px">
  ...
  <PlanningBoard
    v-if="$root.session.writer.selected"
    :key="$root.session.writer.selected.lastupdated"
  />
</template>
...
export default {
  ...
  data() {
    return {
      toollist: this.$root.useObservable(
        this.$root.liveQuery(async () => await this.$root.db.Writer.toArray())
      ),
    };
  },
  methods: {
    ...
    setLiveQuery(item) {
      this.$root.session.writer.selected = this.$root.useObservable(
        this.$root.liveQuery(() => this.$root.db.Writer.get(item.uuid))
      );
    },
  },
  created() {
