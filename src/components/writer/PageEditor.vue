      ...
      <div class="scrolling-toolbar-area" ref="toolbar">
        <template v-if="editor">
          ...
          >
            <svg style="width: 24px; height: 24px" viewBox="0 0 24 24">
              <path
                d="M11,13H21V11H11M11,9H21V7H11M3,5V19H5V5M11,17H21V15H11M3,3H5V1H3A2,2 0 0,0 1,3V21A2,2 0 0,0 3,23H5V21H3V3Z"
              />
            </svg>
          </button>
          <button
          ...
          >
            <svg style="width: 24px; height: 24px" viewBox="0 0 24 24">
              <path
                d="M11,13H21V11H11M11,9H21V7H11M3,5V19H5V5M11,17H21V15H11M5,3H3V1H5A2,2 0 0,1 7,3V21A2,2 0 0,1 5,23H3V21H5V3Z"
              />
            </svg>
          </button>
...
export default {
  ...
  methods: {
    ...
    checkArrowVisibility() {
      ...
      const scrollLeft = Math.round(toolbar.scrollLeft);
      ...
      // Log the math to the console for debugging
      console.log(
        `Remaining Scroll: ${remainingScroll} (Total: ${scrollWidth}, Visible: ${clientWidth}, Scrolled: ${scrollLeft})`
      );

      this.showLeftArrow = scrollLeft > 1;
      this.showRightArrow = remainingScroll > 1;
    },
  },
  mounted() {
    ...
    // Watch for the document data to load
    this.$watch(
      "item",
      (newItem, oldItem) => {
        if (this.editor.isDestroyed) return;

        // This block handles when the file is FIRST loaded into the editor
        if (newItem && !oldItem) {
          this.editor.commands.setContent(newItem.content, false);

          // THE CRITICAL FIX IS HERE:
          // After setting the content, wait for the browser to draw the buttons,
          // then run our check for the arrows.
          this.$nextTick(() => {
            this.checkArrowVisibility();
          });
        }

        // This block handles subsequent updates
        if (newItem) {
    ...
    // Set up the listeners for scroll and resize events
    this.$nextTick(() => {
      const toolbar = this.$refs.toolbar;
      if (toolbar) {
...
.toolbar {
  ...
  /* --- NEW HORIZONTAL SCROLLING RULES --- */
  display: flex; /* Use flexbox for alignment */
  overflow-x: auto; /* This is the key: allows horizontal scrolling */
  white-space: nowrap; /* Prevents buttons from wrapping to a new line */

  /* A nice trick to hide the ugly scrollbar on most browsers */
...
.dropdown-wrapper {
  ...
  display: inline-block; /* Allows it to sit nicely with other buttons */
}
...
.toolbar-group {
  ...
  border-radius: 4px;
  margin-right: 5px; /* Adds a little space between this group and the next button */
}
...
/* This makes the TRIGGER button stay purple when the menu is open */
.toolbar-group .dropdown-trigger.is-open {
  background-color: var(--accent);
  color: var(--accent-f);
  fill: var(--accent-f);
}
...
/* This is the inner area that contains the buttons and scrolls */
...
/* A single style for all buttons in the static part of the bar */
.static-toolbar > button {
  flex-shrink: 0; /* Prevent static buttons from shrinking */
  width: 40px;
  height: 100%;
