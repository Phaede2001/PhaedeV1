<template>
  <div
    class="writer"
    v-bind:class="[$root.lhspin ? 'lhspin' : '', $root.rhspin ? 'rhspin' : '']"
  >
      ...
      <td style="max-width: 200px;">
            ...
            -->
              ...
              <td>
                ...
                <input
                  type="text"
                  class="formInput mantitle"
                  v-model="this.$root.session.writer.selected.title"
                ...
                <input
                  type="text"
                  class="formInput mandesc"
                  v-model="this.$root.session.writer.selected.description"
                  @change="updateDatabase"
                  :placeholder="this.$root.setlang.tools.description"
                />
                <hr />
                {{ this.$root.fullWordCount }}
                {{ this.$root.setlang.writer.words }}
              </td>
            </tr>
          </tbody>
        </table>
 ...
 <button class="clearInterfaceIconButton" @click="calcFullWordCount()">
        ...
        <button
          @click="exportDoc(this.$root.session.writer.selected.files)"
          class="interfaceBtn"
        >
          ...
          {{ this.$root.setlang.writer.docdownload }}
        </button>
    ...
    <div v-if="$root.session.writer.file">
      <div v-if="$root.session.writer.file.type === 'file'">
        <transition name="fade">
          <div class="pageHolder" :key="$root.session.writer.file.uuid">
            <PageEditor
              :pageuuid="$root.session.writer.file.uuid"
              @blur="$root.calcFullWordCount()"
            />
          </div>
        </transition>
      </div>
      ...
      <div
        v-if="$root.session.writer.file.type === 'folder'"
        @change="updateDatabase"
      >
        <div class="titleBar">
          <input
            ...
            v-model="$root.session.writer.file.title"
            @change="updateDatabase"
          />
        </div>
        ...
        <div class="wavemaker_info_box">
          <button
            @click="exportDoc($root.session.writer.file.children)"
            class="interfaceBtn"
          >
            <svg viewBox="0 0 24 24">
              <path
  ...
  <WriterRightSide
    v-if="this.$root.session.writer.file"
    :key="this.$root.session.writer.file.uuid"
  />
  <WriterLeftSide />
</template>
...
export default {
  ...
  methods: {
    async exportDoc(exportArray) {
      ...
      this.exportObject = {};
      let arr = await this.$root.db.Files.where({
        writerid: this.$root.session.writer.selected.uuid,
      }).toArray();
      arr.forEach((file) => {
        this.exportObject[file.uuid] = file;
      });
      this.exportHTML = "";
      this.getfileexport(exportArray);
